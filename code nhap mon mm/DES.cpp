...
string encrypt(string pt, vector<string> rkb, vector<string> rk)
{
	...
	// Straight Permutation Table // HOAN VI P
	...
	for (int i = 0; i < 16; i++) {
		...
		cout << "mo rong nua phai R0: " << right_expanded << endl;
		// XOR RoundKey[i] and right_expanded
		string x = xor_(rkb[i], right_expanded);
		cout << "E(R0) xor k: " << x << endl;
		// S-boxes
		string op = "";
		for (int i = 0; i < 8; i++) {
			...
			val = val % 2;
			op += char(val + '0');
		}cout << "the S-box: " << op<< endl;
		// Straight D-box
		op = permute(op, per, 32);
		cout << "Hoan vi P: " << op<< endl;
		// XOR left and op
		x = xor_(op, left);
		cout << "R = P(C1) xor L0: " << x << endl;
		left = x;
...
int main()
{
	...
	for (int i = 0; i < 16; i++) {
		// Shifting
		cout << "hoan vi PC-1(56bit): " << left << " " << right << endl;
		left = shift_left(left, shift_table[i]);
		right = shift_left(right, shift_table[i]);
		cout << "dich bit: " << left << " " << right << endl;
		// Combining
		string combine = left + right;

		// Key Compression
		string RoundKey = permute(combine, key_comp, 48);
		cout << "PC-2(48bit): " << RoundKey << endl;
		
		rkb.push_back(RoundKey);
		rk.push_back(bin2hex(RoundKey)); cout << endl;
	}
